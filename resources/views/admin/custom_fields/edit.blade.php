...
<form action="{{ route('admin.custome_fields.update', $customeField->id) }}" method="POST">
    ...
    <div class="form-group">
        ...
        <select name="service_id" id="service_id" class="form-control @error('service_id') is-invalid @enderror" required>
            ...
            @foreach ($services as $service)
                <option value="{{ $service->id }}" {{ $service->id === $customeField->service->id ? 'selected' : '' }}>
                    {{ $service->Trans_Name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="form-group">
        ...
        <input type="text" value="{{ old('label_en', $customeField->label_en) }}" name="label_en" class="form-control @error('label_en') is-invalid @enderror" id="label_en" required>
        @error('label_en')
            <small class="invalid-feedback">{{ $message }}</small>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" value="{{ old('label_ar', $customeField->label_ar) }}" name="label_ar" class="form-control @error('label_ar') is-invalid @enderror" id="label_ar" required>
        @error('label_ar')
            <small class="invalid-feedback">{{ $message }}</small>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
            @foreach(['text', 'textarea', 'number', 'select', 'checkbox', 'radio', 'date', 'time', 'file'] as $type)
                <option value="{{ $type }}" {{ $customeField->type == $type ? 'selected' : '' }}>
                    {{ ucfirst($type) }}
                </option>
            @endforeach
        </select>
    ...
    <div id="optionsContainer" style="display: none;">
        ...
        <div id="optionsWrapper">
            ...
            @foreach ($options as $option)
                <input type="text" name="options[]" class="form-control mb-2" value="{{ $option }}" placeholder="{{ __('admin.placeholder_options') }}">
            @endforeach
    ...
    <div class="form-group mt-3">
        ...
        <select name="required" id="required" class="form-control">
            <option value="1" {{ $customeField->required == 1 ? 'selected' : '' }}>{{ __('admin.yes') }}</option>
            <option value="0" {{ $customeField->required == 0 ? 'selected' : '' }}>{{ __('admin.no') }}</option>
        </select>
    </div>
